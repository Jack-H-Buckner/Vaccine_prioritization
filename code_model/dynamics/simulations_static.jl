...
model with the vaccine allocation set at time 1.
...
function simulate(params, μ)
    ...
    for i in 1:T # loop over days
        ...
        # update state
        X = derivitives.mod1_discrete_leftovers(X, # state varabibles S P F I_pre I_asym I_mild D R
            n, # number of groups
            q, # transmission rate
...
function simulate_data(params, μ)
    ...
    for i in 1:T
        ...
        data_disease[i,:] = X
        data_vaccines[i+1,:] = data_vaccines[i,:] + μ1 .* v(i)


        # update state
        X = derivitives.mod1_discrete_leftovers(
            X, # state varabibles S P F I_pre I_asym I_mild D R
            n, # number of groups
            q, # transmission rate
            β_presym, β_asym, β_sym, # transmission rates
            D_pre, D_asym, D_sym, D_exp,# symptom durations
